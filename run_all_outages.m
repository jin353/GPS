...
%% 加载LSTM 64单元结果
load('../results_paper/lstm_64units_results.mat');
lstm_pos_north = error_pos_north_lstm_64;
lstm_pos_east = error_pos_east_lstm_64;
lstm_vel_north = error_vel_north_lstm_64;
lstm_vel_east = error_vel_east_lstm_64;
lstm_yaw = error_yaw_lstm_64;
...
%% 创建图片目录
results_dir = '../results_paper/figures';
...
for idx = 1:length(outage_scenarios)
    ...
    saveas(gcf, fullfile(results_dir, sprintf('Position_%ds.fig', duration)));
    close;
    ...
    saveas(gcf, fullfile(results_dir, sprintf('Velocity_%ds.fig', duration)));
    close;
    ...
    saveas(gcf, fullfile(results_dir, sprintf('Heading_%ds.fig', duration)));
    close;
    ...
    saveas(gcf, fullfile(results_dir, sprintf('Trajectory_%ds.fig', duration)));
    close;
end
