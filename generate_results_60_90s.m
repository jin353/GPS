...
fprintf('╔══════════════════════════════════════════════════════════════╗\n');
fprintf('║           实验结果 - 60秒/90秒中断                          ║\n');
fprintf('║  注意：数据集最大支持91秒中断，无法支持120/180秒           ║\n');
fprintf('╚══════════════════════════════════════════════════════════════╝\n\n');
...
for idx = 1:length(outage_scenarios)
    ...
    lstm_north = abs(ref1_lstm.error_pos_north_loose_LSTM(outage_idx));
    lstm_east = abs(ref1_lstm.error_pos_east_loose_LSTM(outage_idx));
    ...
    lstm_vel_n = abs(ref1_lstm_vel.error_vel_north_loose_LSTM(outage_idx));
    lstm_vel_e = abs(ref1_lstm_vel.error_vel_east_loose_LSTM(outage_idx));
    ...
    lstm_yaw = abs(ref1_lstm_att.error_yaw_loose_LSTM(outage_idx));
...
for idx = 1:length(outage_scenarios)
    ...
    lstm_n = sqrt(mean(abs(ref1_lstm.error_pos_north_loose_LSTM(outage_idx)).^2));
    lstm_e = sqrt(mean(abs(ref1_lstm.error_pos_east_loose_LSTM(outage_idx)).^2));
...
fprintf('\n注意：数据集最大支持91秒中断，无法支持120秒和180秒中断。\n');
fprintf('如需测试更长中断，需要使用更长的数据集。\n');
