...
for k = 1:nn:len
    ...
    if mod(tp,1) == 0
        ...
        % 判断GPS可用性（沿用参考1逻辑）
        % 100-190秒：原始数据3颗卫星，song=0
        % 190-280秒：原始数据4颗卫星，但模拟为中断，song=0
        % 280秒后：原始数据4颗卫星，song=1
        t_rel = tp - t0;
...
%% 保存
save('results/lstm_loose_180s_fixed.mat', ...
